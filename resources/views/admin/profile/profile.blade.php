        ...
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <div class="row">
                        <h6 class="col  d-flex flex-row ">{{ __('Profile') }}</h6>
                        <a href="{{ route('admin.user.admin.profile.password.change') }}" class="text-info col  d-flex flex-row-reverse"><span>{{ __('Password Change') }}</span></a>
                    </div>
                </div>
                <div class="card-body">
                    {{ Form::model($user,['route'=>['admin.user.admin.profile.update',$user->id],'method'=>'POST','id'=>'profile']) }}
                    <div class="row p-2">
                    ...
                    {{ Form::submit('Update',['class'=>'btn btn-primary','id'=>'profile_btn']) }}
                    {{ Form::close() }}
                </div>
            </div>
        </div>
        ...
        <script>
            const updateProfile = ()=>{
                document.getElementById('profile_btn').addEventListener('click',()=>{
                    const validation = new JustValidate('#profile', {
